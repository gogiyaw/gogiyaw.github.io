...
local legit_aa_use = ui_new_checkbox("LUA", "B", "Legit anti-aim on use")
...
ui_new_label("LUA", "B", "Minion-YAW color")
local minion_yaw_clr = ui_new_color_picker("LUA", "B", "Minion yaw color", 255, 166, 180, 255)
...
local function dt_indicator()
    ...
    renderer.text(wnd.x + wnd.w / 2, wnd.y + 9, 255, 255, 255, 255, "c", 0, string.format("Minion DT (%s%%) | ticks: %s", ui_get(ref_dt_hc), ui_get(ref_maxprocessticks)))
...
local function draw()
    ...
    local m_r, m_g, m_b, m_a = ui_get(minion_yaw_clr)
    ...
    renderer.text(w / 2, h / 2 + 40, m_r, m_g, m_b, 255, "c", 0, "MINION-YAW")
